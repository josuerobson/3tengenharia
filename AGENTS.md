...
- `app.requirePermission` aceita `string | string[]` — quando um endpoint é usado por mais de uma tela (cada uma com seu próprio `pageKey`), passe um array: `app.requirePermission(['pageKeyA', 'pageKeyB'], 'READ')`. A checagem é OR (passa se qualquer uma conceder a capacidade), priorizando a de maior alcance (se uma delas for `*_ALL`, o `request.accessScope` resultante não fica own-scoped mesmo que outra seja `*_OWN`). Use isso em vez de re-gated o endpoint pra um pageKey só — foi a causa de pelo menos 4 bugs reais de "tela quebra com 403 pra perfil customizado" nesta sessão (ver linha do tempo no `HANDOFF.md`).
- **UI de página nunca deve decidir privilégio (mostrar seletor de terceiros, botões de escrita, etc.) usando `currentUser.role` (legado)** — use `useAuth()`'s `canReadPage`/`canWritePage`/`isOwnScopedPage(pageKey)`. `role` e perfil de acesso são campos independentes; um usuário pode ter role legado `MANAGER_*` e um perfil de acesso custom bem mais restrito (ex: "Total Pessoal"), e checar só `role` faz a UI oferecer ações que o backend vai rejeitar. Já aconteceu em produção (`TripStartPage.tsx` mostrava seletor de qualquer funcionário pra um perfil "Total Pessoal") — o backend bloqueava a submissão de verdade, mas a UI não devia nem oferecer a opção.
...
- **Exportação de relatórios (PDF + Excel)**: utilitário genérico em `frontend/src/lib/reportExport.ts` (`exportReportToPdf`/`exportReportToExcel`), reutilizado por todos os relatórios do hub "Relatórios" — recebe `columns`/`rows` genéricos, não precisa de código novo por relatório. Excel usa `exceljs` (não `xlsx`/SheetJS — tem 2 CVEs de alta severidade sem correção: prototype pollution e ReDoS), carregado via `import()` dinâmico pelo mesmo motivo do pdf-lib (chunk de ~940KB isolado do bundle principal).
- **Hub de Relatórios (3 níveis)**: `frontend/src/pages/reports/ReportsPage.tsx` é o nível 1 (Sidebar) + nível 2 (abas por módulo, gated por `canReadPage`) + nível 3 (grade de cards). Cada relatório é um componente próprio em `frontend/src/pages/reports/*.tsx` com a assinatura `{ onBack: () => void }`, renderizado inline no lugar da grade (sem rota própria). Os 12 relatórios do sistema estão todos implementados — nenhum "Em breve" restante.
  - **Reaproveitar vs. criar rota dedicada**: se um endpoint existente já retorna exatamente os campos que o relatório precisa, adicione o pageKey `reports.*` como OR no `requirePermission` existente em vez de duplicar rota (ex: "Auditoria 5S por Área" e "Não Conformidades 5S" usam `GET /5s/reports`). Se o relatório precisa de um campo que o endpoint existente não expõe (ex: "Inventário de Ferramentas" precisa de "Última manutenção", que `GET /assets` não retorna), crie uma rota dedicada em `reports.routes.ts` em vez de inflar a rota original só para um caso de uso — evita quebrar o response schema (whitelist do Fastify) de um endpoint já muito consumido.
  - **Campo sem relação FK no schema** (ex: `AssetMaintenanceLog.reportedByUserId`, string solta sem `@relation`): não dá pra usar `include` do Prisma. Padrão: coletar os IDs distintos, um `findMany` em lote pro model de destino, e montar um `Map` pra resolver no `.map()` — evita N+1 de queries individuais.
  - Quando o documento de especificação pede um status que não existe verbatim no schema (ex: "Pendente/Em andamento/Resolvida" quando o model só tem `AGUARDANDO_AVALIACAO/APROVADO/REPROVADO`), documente o mapeamento escolhido no cabeçalho do componente — é uma interpretação, não um fato do sistema.
