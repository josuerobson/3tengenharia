...
- **Hub de Relatórios (3 níveis)**: `frontend/src/pages/reports/ReportsPage.tsx` é o nível 1 (Sidebar) + nível 2 (abas por módulo, gated por `canReadPage`) + nível 3 (grade de cards). Cada relatório é um componente próprio em `frontend/src/pages/reports/*.tsx` com a assinatura `{ onBack: () => void }`, renderizado inline no lugar da grade (sem rota própria). Para reaproveitar um endpoint que já existe em outro módulo em vez de duplicar (ex: "Auditoria 5S por Área" usa `GET /5s/reports`), adicione o pageKey `reports.*` como OR no `requirePermission` existente em vez de criar rota nova.
