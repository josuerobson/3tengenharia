...
- **Hub de Relatórios — Fase 2 pendente**: ver seção 6 para o que já está pronto (Fase 1). Faltam 8 dos 12 relatórios do documento de especificação original (`Modulos Solicitados/Relatorios Nice 2026.txt`, fornecido pelo usuário): Manutenções Preventivas e Realizadas, Histórico de Quilometragem por Veículo (Módulo Veículos); Histórico de Uso por Ferramenta, Inventário de Ferramentas e Equipamentos, Manutenções de Ferramentas (Módulo Ferramentas); Resumo Mensal de Rateio de Horas (Módulo Horas); Não Conformidades 5S – Pendentes e Resolvidas, Evolução 5S por Período (Módulo 5S). O padrão de infraestrutura (rota → service → hub → export) já está estabelecido nos 4 relatórios da Fase 1 — replicar o mesmo padrão, sem precisar reinventar a estrutura. Também não testado interativamente logado no navegador (ver seção 6) — validar o fluxo real antes de divulgar aos usuários finais.
